...
in the 0.1\% \IUPAC{1,1,2,2-perfluoro\|octyl\|trichloro\|silane}
...
\begin{figure}
\includegraphics{Figures/CFD_geometry_flow_cell}\includegraphics{Figures/CFD_geometry_flow_cell_mesh}
...
\begin{figure}
\includegraphics{Figures/CFD_3D_mesh}
...
\begin{figure}
\includegraphics{Figures/CFD_geometry_2D_mesh}

\includegraphics{Figures/CFD_geometry_2D_mesh_closeup}
...
\BPChem{FNIII\_{7-10}} \cite{Michael2003}. Written using the same
...
\begin{figure}
\includegraphics{Figures/CFD_3D_velocity}\includegraphics{Figures/CFD_3D_velocity_xcut}
...
\begin{figure}
\subfloat[\label{fig:CFD FN near surf conc}Near-surface concentration]{\includegraphics{Plots/CFD_FN_DETA_near_surf_conc}



}\subfloat[\label{fig:CFD FN near surf conc rescaled}Same data, rescaled to
show lower concentrations]{\includegraphics{Plots/CFD_FN_DETA_near_surf_conc_rescaled}



}

\includegraphics{Plots/CFD_FN_legend}\caption{\label{fig:CFD Bulk Conc}CFD prediction of the concentration of FN
...
\begin{figure}
\subfloat[\label{fig:CFD FN DETA surf conc}DETA]{\includegraphics{Plots/CFD_FN_DETA_surf_conc}}\subfloat[\label{fig:CFD FN 13F surf conc}13F]{\includegraphics{Plots/CFD_FN_13F_surf_conc}}

\includegraphics{Plots/CFD_FN_legend}
...
\begin{figure}
\includegraphics{Plots/CFD_GO_glass_near_surf_conc}
...
\begin{figure}
\subfloat[\label{fig:CFD GO DETA surf conc}DETA]{\includegraphics{Plots/CFD_GO_DETA_surf_conc}}\subfloat[\label{fig:CFD GO 13F surf conc}13F]{\includegraphics{Plots/CFD_GO_13F_surf_conc}

}

\subfloat[\label{fig:CFD GO glass surf conc}Glass]{\includegraphics{Plots/CFD_GO_glass_surf_conc}

}\includegraphics{Plots/CFD_surf_conc_legend}\caption{\label{fig:CFD GO surface density}Surface density of adsorbed GO
...
13F and SiPEG are shown in Figure 2. The initial rate of adsorption
...
value on the DETA surface at 100 \textbackslash{}u00b5g/ml, while
the highest saturation at 10 \textbackslash{}u00b5g/ml occurred on
13F. On the 13F surface the saturation surface density at 10 \textbackslash{}u00b5g/ml
was nearly as high as 100 \textbackslash{}u00b5g/ml. Adsorption on
the glass surface showed an \textbackslash{}u201covershoot\textbackslash{}u201d
profile in which the surface density reached a maximum and then decreased
gradually with time. 

For all four surfaces and both concentrations, CFD simulations showed
that the concentration of protein in solution near the surface of
the resonator reached 99\% of its final value about 20 seconds after
switching from buffer to protein solution. Representative plots of
the near-surface concentration and adsorbed surface density over time
are shown in the Supplemental Material. The parameter values for the
five models fitted to experimental measurements of the adsorption
of GO on glass, DETA, and 13F are shown in Tables 1, 2, and 3, respectively.
Only the RSA and Langmuir models were fitted to GO on SiPEG, and the
fitted parameter values are shown in Table 4. The experimentally measured
kinetic curves, along with the model that achieved the best fit to
the kinetic data, are shown in Figure 3. 
...
at 10 ug/ml, but not at 100 ug/ml, as shown in Figure 4. The activity
predicted by both models at 100 ug/ml matched the experimental value
much more closely if it was assumed that the protein was active in
both the initial and final states. For GO on DETA, the RSA-derived
models fitted the data better than the Langmuir-type models. The best
fit to the kinetic curves was achieved by the RSA-type model with
post-adsorption transition. However, this model did not predict the
amount of active enzyme at 10 ug/ml or 100 ug/ml, as shown in Figure
4. The two-layer model does not fit the kinetic data quite as well,
but it does predict the amount of active enzyme very well. For GO
on 13F, the two-layer model and the Langmuir-type model with post-adsorption
transition fitted the kinetic data well. Although the two-layer model
fitted the kinetic data slightly better, Figure 4 indicates that it
does not accurately predict the amount of active adsorbed protein.
The Langmuir-type model with a post-adsorption transition predicted
the amount of active protein very well.
...
\begin{figure}
\includegraphics{Figures/Adsorption_Model_Matrix}
...
at 100 \textbackslash{}u00b5g/ml, but the initial rate of adsorption
on the DETA surface was slightly higher at 10 \textbackslash{}u00b5g/ml.

In a previous study, glucose oxidase was adsorbed onto plasma-polymerized
thin films of hexamethyldisiloxane (HDMS) on silica substrates.9 HDMS
is a hydrophobic polymer, and its surface properties were modified
by exposure to nitrogen and oxygen plasma, as shown in Table 5. Adsorption
was measured with a quartz crystal microbalance (QCM), although only
the saturation values were quantified. The size and shape of adsorbed
GO was also measured by AFM. It was assumed that GO in solution could
be approximated by an ellipsoid with a major axis of 10-14 nm and
a minor axis of 6-8 nm. It was found that GO adsorbed to native HDMS
surfaces with the major axis parallel to the surface, while GO adsorbed
to HDMS-O and HDMS-N with the major axis normal the surface. The maximum
area occupied by an adsorbed molecule (its \textbackslash{}u201cfootprint\textbackslash{}u201d)
would be 88nm2 when adsorbed with major axis parallel to surface,
while the minimum area would be 28 nm2 when adsorbed with the major
axis normal to surface. Previous AFM studies of GO adsorbed on gold
also found that it could be represented by an ellipse with a major
axis of 14-18 nm and a minor axis of 5-8 nm21.


\subsubsection{Using models to interpret experimental data}
...
activity at a solution concentration of 10\textbackslash{}u00b5g/ml.
When the solution concentration was increased to 100 \textbackslash{}u00b5g/ml,
almost all of the adsorbed enzyme remained active. Both of the two-stage
models predicted the amount of active enzyme at 10 \textbackslash{}u00b5g/ml,
but under-predicted the amount of active enzyme at 100 \textbackslash{}u00b5g/ml
(assuming that enzyme in state a is active and enzyme in state \textbackslash{}u00df
is inactive). If it was assumed that enzyme in both states remains
active, both of the two-stage models would accurately predict the
amount of active enzyme at 100 \textbackslash{}u00b5g/ml. It has been
shown that some \textbackslash{}u201csoft\textbackslash{}u201d enzymes
lose most of their structure and activity when adsorbed to glass,
while \textbackslash{}u201chard\textbackslash{}u201d enzymes retain
their structure and function after adsorption22, 23. An alternative
...
enzyme at 10 \textbackslash{}u00b5g/ml, and the high fraction of active
enzyme at 100 \textbackslash{}u00b5g/ml. Modeling this process would
require a model that incorporated both a post-adsorption transition
and multi-layer adsorption.
...
assay at 10 \textbackslash{}u00b5g/ml. When the solution concentration
was increased to 100 \textbackslash{}u00b5g/ml, approximately half
of the adsorbed GO apparently lost its activity. It is unlikely that
this reduction was caused by denaturation, since adsorbed proteins
are more likely to retain their native conformation when the rate
of adsorption is higher24. Further, the fitted parameter values for
the two-stage RSA-type model were fairly implausible. The fitted transition
rate constant was two orders of magnitude higher than the transition
rate constant for the glass surface, and the surface area occupied
by a molecule in state \textbackslash{}u00df was 16 times larger than
a molecule in state a. A more likely hypothesis for the 50\% reduction
in activity is that a second layer of adsorbed enzyme formed, preventing
the substrate solution from reaching the lower layer of enzyme. The
two-layer model provided the best prediction of the amount of active
adsorbed enzyme at both concentrations, although it did not fit the
kinetic data quite as well as the RSA-type two-stage model. 
...
enzyme was active at a solution concentration of 10\textbackslash{}u00b5g/ml.
When the solution concentration was increased to 100\textbackslash{}u00b5g/ml,
...
10\textbackslash{}u00b5g/ml. At 100\textbackslash{}u00b5g/ml, GO adsorbed
on the SiPEG surface, but at a relatively low surface density. The
enzyme activity assay did not detect any significant amount of active
enzyme for either solution concentration. This may be because the
amount of activity was below the detection limit of the assay, or
because the enzyme did not retain its activity on SiPEG. It is well
known that SiPEG surfaces resist protein adsorption, but denaturation
on SiPEG has not been previously reported. Further investigation is
necessary to answer this question. Both the RSA and Langmuir single-layer
models fitted this data set well.

Comparison of adsorption models \textbackslash{}u2022 Fitting models
to data allows us to test hypotheses about mechanisms of adsorption
\textbackslash{}u2022 Sum of squared error is a only a rough guide
to gauging the usefulness of a model \textbackslash{}u2022 Activity
assay provides valuable data to support or reject a particular model
\textbackslash{}u2022 Langmuir-type and RSA-type models predict very
different mechanisms \textbackslash{}u2022 RSA-type models assume
proteins occupy a spherical footprint on surface, but the assumption
works well \textbackslash{}u2022 Opportunity and difficulty of creating
two-layer models with post-adsorption transition
\end{document}
