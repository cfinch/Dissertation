...
currentprojection=orthographic(-2,1,4); // Coefficient a
...
// Coefficient a
xaxis3("$h$", Bounds(Both, Min), ticks=InTicks);

yaxis3(Label("$\theta$", position=(0.5,0), align=Relative((1.0,-0.5))), 0, theta_max,
    axis=Bounds(Both, Min), ticks=InTicks(beginlabel=false, endlabel=true));
yaxis3(axis=XZEquals(2, 0), ticks=OutTicks(ticklabel=noLabels));

zaxis3(Label("$k_1$", position=(0.5,0), align=Relative((0.75,0.0))), 
